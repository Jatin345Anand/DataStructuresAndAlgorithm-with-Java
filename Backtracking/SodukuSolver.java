...
//	public static 
	public static ArrayList<Integer> takenumberfromHashMap(int r,int c,HashMap<String, ArrayList<Integer>> possibleNumbersforVacentbox,int subNumber){
...
//		print2DArray();
		...
		for(int i=0;i<keysarr.size();i++) {
			...
			if(Integer.parseInt(keysarr.get(i).substring(0, 1))==r  && Integer.parseInt(keysarr.get(i).substring(1, 2))==c) {
				key = keysarr.get(i);
				break;
			}
...
//		}
    ...
    public static boolean isSodukoSolver(int[][] subGrid, HashMap<String, ArrayList<Integer>> possibleNumbersforVacentbox,int subGridNumber) {
    	boolean ans = true;
...
//		takeAllPossibleNumbersforVacentBox(indexRnumber, indexCnumber, board);
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 9; i++) {
		  ...
		  System.out.println("is soduku solver = "+ sudokuSolver(GRID));
		scanner.close();
	}
}